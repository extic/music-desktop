...
export function focusOnModalOnly(modalSelector: string): void {
  ...
  // const focusableElements = 'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';
  const modalFocusableElements = modal.querySelectorAll(focusableElements) as unknown as HTMLElement[];
...
*:focus {
  outline: none;
  border: 1px solid #53adcd !important;
}
...
.generic-dialog {
  ...
  .dialog-container {
    ...
    &.dialog-container-enter-active {
      animation: dialogIn 0.3s cubic-bezier(0.79, -0.49, 0.33, 1.46);
    }
    &.dialog-container-leave-active {
      animation: dialogOut 0.3s cubic-bezier(0.79, -0.49, 0.33, 1.46);
    }
